...
from Code_treegan.evaluation.FPD import calculate_fpd, calculate_activation_statistics

from Code_treegan.metrics import *
from Code_treegan.loss import *

from Code_treegan.evaluation.pointnet import PointNetCls
...
import os
import copy
from Code_treegan.utils.common_utils import *
# from arguments import Arguments

# 文件： 评估
# 要把最好的一个epoch作为rk_plane.pt
# 改动完后，循环读出来， 发现最低值出现在第80个epoch，于是把这个设置为rk_plane.pt
'''
python eval_treegan.py --dataset_path ~/Data/CRN --save_sample_path 
./rk_treegan_saved_results --model_pathname ./rk_pretrain_checkpoints/plane/tree_ckpt_0_multi.pt --gpu 1
...
def checkpoint_eval(G_net, device, n_samples=5000, batch_size=100,conditional=False, ratio='even', FPD_path=None, class_choices=None):
    ...
    fpd = calculate_fpd(fake_pcs, statistic_save_path=FPD_path, batch_size=batch_size, dims=1808, device=device)
    # print(fpd)
...
def test(args, mode='FPD', verbose=True):
    ...
    if mode == 'save':
        ...
        fpd = calculate_fpd(fake_pcs, statistic_save_path=args.FPD_path, batch_size=args.batch_size, dims=1808, device=args.device)
        if verbose:
