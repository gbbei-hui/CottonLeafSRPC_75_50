...
# CSDN 上：
#  先 eval_treegan.py: 得到最好权重，  trainer.py 进行测试  进行评估eval_completion.py
'''
--eval_with_GT true 
--saved_results_path rk_saved_results/CRN_plane
'''



#遇到的问题：阈值设置
# min_dists[min_dists < thre]的意思是取min_dists中点的值小于阈值的点。
# 我跑的时候发现报错了一个除数为零的问题，排查出来就出现在这个阈值上，表达的意思是之前训练输出保存下来的点离实际位置太远了
